...
5. Run `npm run bot:start` on a continuously running server to answer `/start`, `/app`, `/help`, and `/tips` with illustrated messages.

The welcome, help, and tips messages use the deployed Mini App artwork from its public `/assets` directory. Run `npm run bot:setup` after command changes, then restart the bot process.
